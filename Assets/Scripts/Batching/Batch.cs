...
public class Batch : MonoBehaviour {
  ...
  const int MAX_VERTS_HARD = 0xfffe;    // This is the Unity limit
        ...
        /// <summary>
        /// í«ãLïîï™ÅBAudioVizÇ…çáÇÌÇπÇƒäGãÔÇÃScaleÇïœÇ¶ÇÈ
        /// </summary>
        int batchCount;
        float scaleMultiPlayer = 3f;
        float positionY;

        private void OnEnable()
        {
            //batchCount = transform.parent.childCount % AudioViz.AudioBandBuffer.Length;
            batchCount = MusicPitchInt.pitchNum;
            positionY = transform.position.y;
        }
        private void Update()
        {
            positionY = AudioViz.AudioBandBuffer[batchCount] * scaleMultiPlayer;

            if (!ReferenceEquals(positionY, null) | !ReferenceEquals(positionY, "")) {
